...
function FocusSearchLocation({ location, request }) {
  const map = useMap()
  useEffect(() => {
    if (location && request > 0) map.flyTo(location, 12, { duration: .45 })
  }, [location, request, map])
  return null
}
...
function BoulderMap({ spots, selectedSpot, onSelect, onDismiss, userLocation, locationFocusRequest, searchLocation, searchLocationFocusRequest, spotFocusRequest: externalSpotFocusRequest, spotCenterRequest, activities, plans, showVisitMarkers, onSelectPlan, onActivityBoundsChange }) {
  ...
  return (
    <div className="map-frame">
      <MapContainer center={initialView ? [initialView.latitude, initialView.longitude] : mannheimCenter} zoom={initialView?.zoom ?? 13} zoomControl={false} scrollWheelZoom className="map-canvas">
        ...
        <FocusSearchLocation location={searchLocation} request={searchLocationFocusRequest} />
        <MapViewportResize />
...
export function MapView({ spots, currentUser, selectedId, spotFocusRequest = 0, lastVisitedSpotId, onSelectSpot, onVisit, onPlan, onReport, onOpenUserFeed, onOpenPlanFeed, onOpenMessage, query, setQuery, filter, setFilter, isPickingSpot, onCancelPicker, onMessage }) {
  ...
  return (
    <main className="view map-view">
      ...
      <BoulderMap spots={visibleSpots} selectedSpot={selectedSpot} onSelect={(spotId) => { setSelectedMapPlan(null); onSelectSpot(spotId) }} onSelectPlan={(plan) => { setSelectedMapPlan(plan); onSelectSpot(null) }} onDismiss={() => { if (!isPickingSpot) { setSelectedMapPlan(null); onSelectSpot(null) } }} userLocation={userLocation} locationFocusRequest={locationFocusRequest} searchLocation={searchLocation} searchLocationFocusRequest={searchLocationFocusRequest} spotFocusRequest={spotFocusRequest} spotCenterRequest={spotCenterRequest} activities={showVisitMarkers ? activities.filter((activity) => (!locationSpotIds || locationSpotIds.has(String(activity.spot_id))) && (!showPlanned || !plannedSpotIds.has(String(activity.spot_id)))) : []} plans={showPlanned ? mapPlans.filter((plan) => !locationSpotIds || locationSpotIds.has(String(plan.spot_id))) : []} showVisitMarkers={showVisitMarkers} onActivityBoundsChange={setActivityBounds} />
