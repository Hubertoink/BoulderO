...
function GroupSpotMapPicker({ spots, selectedIds, maxSelection = 5, title = 'Auf Karte auswählen', onApply, onClose }) {
  ...
  return <div className="composer-backdrop group-spot-map-backdrop" role="presentation">
    <section className="group-spot-map-dialog" role="dialog" aria-modal="true" aria-label="Hallen auf der Karte auswählen">
      ...
      <label className="group-spot-map-search"><IconSearch size={17} /><input type="search" name="spot-search" autoComplete="off" autoCorrect="off" spellCheck={false} enterKeyHint="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Halle oder Ort suchen" />{search && <button type="button" onClick={() => setSearch('')} aria-label="Hallensuche löschen"><IconX size={15} /></button>}</label>
...
function GroupDetailView({ groupId, spots, onBack, onOpenSpot, onOpenUserFeed, onSummaryChange, onNotificationsRead = async () => {} }) {
  ...
  const canReadNotifications = ['active', 'invited'].includes(group?.membership_status)
  ...
  async function markGroupNotificationsRead(targetTab) {
    try {
      await api(`/api/community/groups/${groupId}/notifications/read`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tab: targetTab }) })
      setGroup((current) => current ? { ...current, [`unread_${targetTab}`]: 0 } : current)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => void markGroupNotificationsRead(tab), 1000)
    return () => window.clearTimeout(timer)
  }, [canReadNotifications, groupId, tab])
  ...
  function selectTab(next) { setTab(next); const params = new URLSearchParams({ group: groupId, tab: next }); const list = new URLSearchParams(window.location.search).get('list'); if (list) params.set('list', list); window.history.replaceState(window.history.state, '', `/groups?${params}`) }
...
export function GroupsView({ spots, onOpenFriends, onOpenSpot, onOpenUserFeed, onSummaryChange, notificationCounts = {}, onNotificationsRead = async () => {} }) {
  const initialParameters = new URLSearchParams(window.location.search)
  const initialGroupId = initialParameters.get('group')
  const [selectedGroupId, setSelectedGroupId] = useState(initialGroupId)
  const [tab, setTabState] = useState(['mine', 'discover', 'invites'].includes(initialParameters.get('list')) ? initialParameters.get('list') : 'mine')
  ...
  function setTab(nextTab) { setTabState(nextTab); window.history.replaceState(window.history.state, '', `/groups${nextTab === 'mine' ? '' : `?list=${nextTab}`}`) }
  function openGroup(id, nextTab = 'overview') { setSelectedGroupId(id); const list = tab === 'mine' ? '' : `&list=${tab}`; window.history.replaceState(window.history.state, '', `/groups?group=${encodeURIComponent(id)}&tab=${nextTab}${list}`) }
  function backToGroups() { setSelectedGroupId(null); window.history.replaceState(window.history.state, '', `/groups${tab === 'mine' ? '' : `?list=${tab}`}`); void load() }
  if (selectedGroupId) return <GroupDetailView groupId={selectedGroupId} spots={spots} onBack={backToGroups} onOpenSpot={onOpenSpot} onOpenUserFeed={onOpenUserFeed} onSummaryChange={onSummaryChange} onNotificationsRead={onNotificationsRead} />
  const display = tab === 'mine' ? groups : tab === 'discover' ? discover : invitations
  return <main className="view content-view compact-view social-view groups-view"><section className="social-section friends-section"><div className="section-heading"><h2>Community</h2><div className="friends-tabs"><button type="button" onClick={onOpenFriends}>Freunde</button><button className="is-active">Gruppen{Number(notificationCounts.unread_groups) > 0 && <b>{Number(notificationCounts.unread_groups) > 9 ? '9+' : notificationCounts.unread_groups}</b>}</button></div></div><div className="groups-toolbar"><div className="friends-tabs"><button className={tab === 'mine' ? 'is-active' : ''} onClick={() => setTab('mine')}>Meine</button><button className={tab === 'discover' ? 'is-active' : ''} onClick={() => setTab('discover')}>Entdecken</button><button className={`${tab === 'invites' ? 'is-active ' : ''}has-badge`} onClick={() => setTab('invites')}>Einladungen{Number(notificationCounts.unread_group_invitations) > 0 && <b>{Number(notificationCounts.unread_group_invitations) > 9 ? '9+' : notificationCounts.unread_group_invitations}</b>}</button></div><button type="button" onClick={() => setCreating(true)}><IconPlus size={16} />Gruppe erstellen</button></div>{error && <p className="form-error">{error}</p>}{tab === 'discover' && <label className="search-field group-search"><IconSearch size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Gruppen, Ort oder Beschreibung suchen" />{query && <button type="button" onClick={() => setQuery('')} aria-label="Suche löschen"><IconX size={16} /></button>}</label>}<div className="group-list">{!display.length && <p className="journal-empty">{tab === 'mine' ? 'Du bist noch in keiner Gruppe. Erstelle eine Gruppe oder entdecke Communities in deiner Nähe.' : tab === 'invites' ? 'Keine offenen Gruppeneinladungen.' : 'Keine passenden Gruppen gefunden.'}</p>}{display.map((group) => { const unreadCount = Number(group.unread_notifications ?? 0); return <article key={group.id} className={group.image_url ? 'has-image' : ''} style={groupCoverStyle(group)}><button type="button" className="group-list__main" onClick={() => openGroup(group.id)}><span><b>{group.name}</b><small>{group.city || 'BoulderO Gruppe'} · {group.member_count} Mitglied{group.member_count === 1 ? '' : 'er'}</small><em>{group.description || accessCopy[group.access_mode]?.[1]}</em>{group.next_event?.starts_at && <i><IconCalendarEvent size={14} />{formatPlanDate(group.next_event.starts_at)} · {group.next_event.spot_name}</i>}</span><IconChevronRight size={18} /></button>{tab === 'mine' && unreadCount > 0 && <b className="group-list__badge">{unreadCount > 9 ? '9+' : unreadCount}</b>}{tab === 'invites' && <button type="button" onClick={() => openGroup(group.id)}>Einladung ansehen</button>}</article>})}</div></section>{creating && <GroupEditorDialog spots={spots} onClose={() => setCreating(false)} onSaved={async (id) => { setCreating(false); await load(); openGroup(id) }} />}</main>
}
